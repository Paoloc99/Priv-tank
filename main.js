...
function startGame() {
  ...
  const createScene = async function () {
    ...
    if (debug) {   
      /** DEBUG */
      // Aggiungi la gestione delle frecce direzionali per la camera
      ...
      // Aggiungi la gestione del movimento attraverso le frecce direzionali
      ...
      // Aggiungi la gestione del movimento della camera con il mouse
      ...
      // Funzione di aggiornamento per il movimento
      scene.registerBeforeRender(function () {
        var cameraRadius = camera.radius;
        ...
        camera.radius = Math.max(cameraRadius, 1); // Limita il raggio minimo
      });
    }
    // light setup
